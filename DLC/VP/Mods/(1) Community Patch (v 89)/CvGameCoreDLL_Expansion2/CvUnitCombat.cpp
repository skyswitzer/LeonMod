			...
			if(pCity)
			{
				if(pkAttacker)
				{
					...
					else if(pkAttacker->getOwner() == GC.getGame().getActivePlayer() && (pCity->getOwner() != pkAttacker->getOwner()))
					{
						...
						//red icon over attacking unit
						GC.messagePlayer(uiParentEventID, pkAttacker->getOwner(), false, GC.getEVENT_MESSAGE_TIME(), strBuffer/*, "AS2D_COMBAT", MESSAGE_TYPE_INFO, pkAttacker->m_pUnitInfo->GetButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pkAttacker->getX(), pkAttacker->getY(), true, true*/);
					}
#endif
#if defined(MOD_BALANCE_CORE_MILITARY)
			...
			if(pkDefender)
			{
				if(pkAttacker)
				{
					...
					else if(pkDefender->IsDead())
					{
						...
						{
							strBuffer = GetLocalizedText("TXT_KEY_MISC_YOU_ARE_ATTACKED_BY_AIR_AND_DEATH", pkDefender->getNameKey(), pkAttacker->getNameKey());
							GC.messagePlayer(uiParentEventID, pkDefender->getOwner(), true, GC.getEVENT_MESSAGE_TIME(), strBuffer/*, "AS2D_COMBAT", MESSAGE_TYPE_INFO, pkDefender->getUnitInfo().GetButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pkTargetPlot->getX(), pkTargetPlot->getY()*/);
						}
					...
					if(pCity->getOwner() != NO_PLAYER)
					{
						...
						if(iActivePlayerID == pCity->getOwner())
						{
							...
							else if (iInterceptionDamage <= 0)
							{
								...
								GC.messagePlayer(uiParentEventID, pCity->getOwner(), true, GC.getEVENT_MESSAGE_TIME(), strBuffer/*, "AS2D_COMBAT", MESSAGE_TYPE_INFO, pkDefender->getUnitInfo().GetButton(), (ColorTypes)GC.getInfoTypeForString("COLOR_RED"), pkTargetPlot->getX(), pkTargetPlot->getY()*/);
							}
						}
					}
#endif
	...
	if (pInterceptor) {
		...
		if (iInterceptionDamage > 0) {
			...
			if (GC.getGame().getActivePlayer() == paraUnit.getOwner()) {
				...
				GC.messagePlayer(uiParentEventID, paraUnit.getOwner(), true, GC.getEVENT_MESSAGE_TIME(), strBuffer);
			}
