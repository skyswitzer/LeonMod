...
// Message wrappers
#define SHOW_PLAYER_MESSAGE(pPlayer, szMessage)       if (pPlayer) GC.messagePlayer(0, pPlayer->GetID(), false, GC.getEVENT_MESSAGE_TIME(), szMessage)
#define SHOW_CITY_MESSAGE(pCity, ePlayer, szMessage)  if (pCity) GC.messageCity(0, pCity->GetIDInfo(), ePlayer, false, GC.getEVENT_MESSAGE_TIME(), szMessage)
#define SHOW_UNIT_MESSAGE(pUnit, ePlayer, szMessage)  if (pUnit) GC.messageUnit(0, pUnit->GetIDInfo(), ePlayer, false, GC.getEVENT_MESSAGE_TIME(), szMessage)
#define SHOW_PLOT_MESSAGE(pPlot, ePlayer, szMessage)  if (pPlot) GC.messagePlot(0, pPlot->GetPlotIndex(), ePlayer, false, GC.getEVENT_MESSAGE_TIME(), szMessage)
#define SHOW_PLOT_POPUP(pPlot, ePlayer, szMessage) \
if (pPlot) \
{ \
	pPlot->showPopupText(ePlayer, szMessage); \
}\
