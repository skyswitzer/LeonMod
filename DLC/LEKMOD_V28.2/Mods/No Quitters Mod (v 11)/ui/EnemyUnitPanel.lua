...
function UpdateCombatOddsUnitVsCity(pMyUnit, pCity)
	...
	if (pMyUnit ~= nil) then
		...
		if (iMyStrength > 0) then
			...
			-- Tourism
			iModifier = pMyUnit:GetTourismCombatPenalty(iTheirPlayer);
			if (iModifier ~= 0) then
				controlTable = g_MyCombatDataIM:GetInstance();
...
-- TODO: African Forest Elephant
			...
			if (pTheirUnit:IsCombatUnit()) then
				...
				-- Tourism
				iModifier = pTheirUnit:GetTourismCombatPenalty(iMyPlayer);
				if (iModifier ~= 0) then
					controlTable = g_TheirCombatDataIM:GetInstance();
					controlTable.Text:LocalizeAndSetText("TXT_KEY_EUPANEL_EMPIRE_INFLUENCED_PENALTY");
					controlTable.Value:SetText( GetFormattedText(strText, iModifier, false, true) );
...
function UpdateCombatOddsCityVsUnit(myCity, theirUnit)
	...
	if (theirUnit:IsCombatUnit()) then
		...
		-- Tourism
		iModifier = theirUnit:GetTourismCombatPenalty(myPlayerID);
		if (iModifier ~= 0) then
			controlTable = g_TheirCombatDataIM:GetInstance();
			controlTable.Text:LocalizeAndSetText("TXT_KEY_EUPANEL_EMPIRE_INFLUENCED_PENALTY");
			controlTable.Value:SetText( GetFormattedText(strText, iModifier, false, true) );
